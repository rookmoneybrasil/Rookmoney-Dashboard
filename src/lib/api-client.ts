...
export interface DashboardData { userName: string; monthBalance: number; monthIncome: number; monthExpense: number; incomeChange: number; expenseChange: number; totalReceivable: number; totalPeopleReceivable: number; totalIncomeReceivable: number; recentTransactions: Transaction[]; goals: Goal[]; upcomingBills: Bill[]; upcomingPersonPayables: UpcomingPersonPayable[]; upcomingPeopleReceivable: UpcomingPersonPayable[]; pendingBillsCount: number; pendingBillsAmount: number; personPayablesAmount: number; overdueCount: number; overBudgetCount: number; healthScore: number; projections: ProjectionMonth[]; mood: string; monthlyHistory: MonthlyHistory[]; topCategories: TopCategory[]; insight: string }
