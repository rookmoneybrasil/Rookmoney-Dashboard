...
function BillsPopup({ month, peopleDue, onClose }: { month: ProjectionMonth; peopleDue: PersonDue[]; onClose: () => void }) {
  ...
  const blocks = [
    ...
    ...(peopleDue.length > 0 ? [{ label: 'Pessoas', icon: Users, color: 'text-purple-400', bg: 'bg-purple-500/10', border: 'border-purple-500/20', total: iOweTotal, count: peopleDue.length }] : []),
  ].filter(b => b.count > 0)
      ...
      <div className="relative bg-ink-800 border border-ink-600 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[85vh] flex flex-col shadow-2xl"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/6 shrink-0">
          <div>
            ...
            <p className="text-sm text-danger font-semibold mt-0.5">{`-${formatCurrency(month.amount + iOweTotal)}`}</p>
          </div>
    ...
    <>
      <div className="bg-ink-800 rounded-xl border border-ink-700 p-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          {months.map(m => {
            const displayAmount = m.amount + iOweTotal
            return (
            <button key={m.label} onClick={() => setSelected(m)}
              ...
              <div className="flex flex-col gap-0.5">
                ...
                {iOweTotal > 0 && <p className="text-[10px] text-slate-600">👥 {formatCurrency(iOweTotal)} pessoas</p>}
              </div>
